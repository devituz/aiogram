...
CHANNELS = [
    {"id": -1002345678901, "username": "@Vertual_Bola"},
]
...
async def is_subscribed(user_id: int) -> bool:
    for ch in CHANNELS:
        ...
        try:
            ...
            print(f"❌ Obuna tekshirishda xato ({username}): {e}")
            return False
    return True
...
async def check_user_requirements(message: Message) -> bool:
    ...
    for ch in CHANNELS:
        ...
        try:
            member = await bot.get_chat_member(chat_id=chat_id, user_id=user_id)
            if member.status not in ["member", "administrator", "creator"]:
                not_subscribed_channels.append(username)
        except Exception as e:
            print(f"❌ {username} tekshirishda xato: {e}")
            not_subscribed_channels.append(username)
...
async def check_subscription(callback: CallbackQuery):
    ...
    if not_subscribed:
        ...
        await callback.message.answer(
            "⚠️ Quyidagi kanallarga obuna bo‘lmagansiz:",
            reply_markup=keyboard
        )
        return
